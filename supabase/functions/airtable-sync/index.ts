...
serve(async (req: Request) => {
  ...
  try {
    ...
    // NOTE: Slugs are now generated by database trigger when records come from Google AppScript
    // airtable-sync should NOT generate or modify slugs anymore
    ...
    const supabaseData = {
      ...
      // NEVER OVERWRITE slug - it's generated by database trigger from Google AppScript data
      // Once a slug is created, it cannot be changed by airtable-sync
      slug: neverOverwrite(existingRecord?.slug, existingRecord?.slug),
      ...
      // liga_web is computed from slug by database trigger, preserve existing value
      liga_web: existingRecord?.liga_web || null,
